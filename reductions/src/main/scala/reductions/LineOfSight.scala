...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.indices.foreach(i => output(i) = if(i == 0) input(i) else max(output(i - 1), input(i) / i))
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from until until).map(i => if(i == 0) 0 else input(i) / i).max
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val hs = (end - from) / 2
      val (l, r) = parallel(upsweep(input, from, from + hs, threshold), upsweep(input, from + hs, end, threshold))
      Node(l, r)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    (from until until).foreach(i => output(i) = max(input(i) / i, if(i == from) startingAngle else output(i - 1)))
  }
